...
int internal_1_bits;                // Number of bits for level 2 page table in virtual address
int internal_2_bits;                // Number of bits for level 3 page table in virtual address
int internal_3_bits;                // Number of bits for level 4 page table in virtual address
int external_bits;                  // Number of bits for (level 1) page dirrectory in virtual address
...
// Function to get level 3 page_table's bits
int get_internal_3_bits(void* va) {
    unsigned long mask = (1 << internal_3_bits) - 1;
    return ((unsigned long)va >> offset_bits) & mask;
}

// Function to get level 2 page_table's bits
int get_internal_2_bits(void* va) {
    unsigned long mask = (1 << internal_2_bits) - 1;
    int shift_bits = offset_bits + internal_3_bits;
    return ((unsigned long)va >> shift_bits);
}

// Function to get level 1 page_table's bits
int get_internal_1_bits(void* va) {
    unsigned long mask = (1 << internal_1_bits) - 1;
    int shift_bits = offset_bits + internal_3_bits + internal_2_bits;
    return ((unsigned long)va >> shift_bits);
}

// Function to get page directory's bits
int get_external_bits(void* va) {
    unsigned long mask = (1 << external_bits) - 1;
    int shift_bits = offset_bits + internal_3_bits + internal_2_bits + internal_1_bits;
    return ((unsigned long)va >> shift_bits);
}
...
void set_physical_mem() {
    ...
    internal_1_bits = log2(PGSIZE/sizeof(pte_t));
    internal_2_bits = internal_1_bits;
    internal_3_bits = internal_1_bits;
    external_bits = 48 - offset_bits - internal_1_bits - internal_2_bits - internal_3_bits;
...
/*
 * Function to add a virtual to physical page translation to the TLB.
 */
...
/*
 * Function to check TLB for a valid translation.
...
/*
 * Function to print TLB miss rate.
 */
...
pte_t *translate(pde_t *pgdir, void *va) {
    ...
    int page_table_1_index = get_internal_1_bits(va);
    int page_table_2_index = get_internal_2_bits(va);
    int page_table_3_index = get_internal_3_bits(va);
    ...
    if (page_dir[page_dir_index].valid == 0) {
        ...
        page_table_entry* pt1 = page_dir[page_dir_index].PPN;
        // Check if the page table entry is valid
        if (pt1[page_table_1_index].valid == 0) {
            return NULL;
        } else {
            page_table_entry* pt2 = pt1[page_table_1_index].PPN;
            if (pt2[page_table_2_index].valid == 0) {
                return NULL;
            } else {
                page_table_entry* pt3 = pt2[page_table_2_index].PPN;
                if (pt3[page_table_3_index].valid == 0) {
                    return NULL;
                } else {
                    // If page table entry is valid then
                    // add the Physical address to the TLB
                    add_TLB(base_va, pt3[page_table_3_index].PPN);
                    return (pte_t*)(((unsigned long) pt3[page_table_3_index].PPN) + offset);
                }
            }
        }
    }
...
int page_map(pde_t *pgdir, void *va, void *pa) {
    ...
    if (page_dir[page_dir_index].valid == 0) {
        int pt_size = pow(2, internal_1_bits) * sizeof(page_table_entry);
        page_dir[page_dir_index].PPN = (page_table_entry*) malloc(pt_size);
        page_dir[page_dir_index].valid = 1;
    }
    page_table_entry* pt1 = page_dir[page_dir_index].PPN;
    if (pt1[page_table_1_index].valid == 0) {
        int pt_size = pow(2, internal_2_bits) * sizeof(page_table_entry);
        pt1[page_table_1_index].PPN = (page_table_entry*) malloc(pt_size);
        pt1[page_table_1_index].valid = 1;
    }
    page_table_entry* pt2 = pt1[page_table_1_index].PPN;
    if (pt2[page_table_2_index].valid == 0) {
        int pt_size = pow(2, internal_3_bits) * sizeof(page_table_entry);
        pt2[page_table_2_index].PPN = (page_table_entry*) malloc(pt_size);
        pt2[page_table_2_index].valid = 1;
    }

    // Add the page table entry
    page_table_entry* pt3 = pt2[page_table_2_index].PPN;
    pt3[page_table_3_index].PPN = pa;
    pt3[page_table_3_index].valid = 1;
...
void t_free(void *va, int size) {
    ...
    for (int i=virt_base_index; i<virt_end_index; i++) {
        ...
        page_table_entry* pt2 = pt1[page_table_1_index].PPN;
        page_table_entry* pt3 = pt2[page_table_2_index].PPN;
        pt3[page_table_3_index].valid = 0;
        virtual_bitmap[i] = 0;
    }
    // Critical Section End
    pthread_mutex_unlock(&memory_lock);
